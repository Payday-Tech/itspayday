  ...
  return (
    <div className="modal-overlay" onClick={handleBackdropClick}>
      <div className="modal-content">
        ...
        <h3>Get in touch</h3>
        <p className="small">For onboarding, partnerships, support, or compliance queries.</p>

        {submitted ? (
          <div style={{ textAlign: 'center', padding: '20px' }}>
            <p style={{ color: 'var(--color-brand-700)', fontWeight: 600 }}>Thanks. Our team will respond shortly.</p>
          </div>
        ) : (
          <form className="form-grid" onSubmit={handleSubmit}>
            <div>
              <label htmlFor="modal-name">Name</label>
              <input id="modal-name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: sanitizeInput(e.target.value, 100) })} required disabled={submitting} />
              {errors.name && <div className="helper-text" style={{ color: 'var(--color-error)' }}>{errors.name}</div>}
            </div>
            <div>
              <label htmlFor="modal-email">Email</label>
              <input id="modal-email" type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: sanitizeInput(e.target.value, 254) })} required disabled={submitting} />
              {errors.email && <div className="helper-text" style={{ color: 'var(--color-error)' }}>{errors.email}</div>}
            </div>
            <div>
              <label htmlFor="modal-topic">Topic</label>
              <select id="modal-topic" value={formData.topic} onChange={(e) => setFormData({ ...formData, topic: e.target.value })} disabled={submitting}>
            ...
            <div>
              ...
              <textarea id="modal-message" rows={4} value={formData.message} onChange={(e) => setFormData({ ...formData, message: sanitizeInput(e.target.value, 2000) })} required disabled={submitting} />
